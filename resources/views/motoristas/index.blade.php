<x-layouts.app title="Motoristas">

    <div class="mt-4 flex flex-wrap items-start justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Motoristas</h2>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Gerencie o cadastro de motoristas.</p>
        </div>
    ...
    <form id="filter-form" method="GET" action="{{ route('motoristas.index') }}" class="mt-6">
        <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="flex min-w-56 flex-1 overflow-hidden rounded-lg border shadow-xs
                ...
                <input type="text" name="search" value="{{ $currentSearch }}" placeholder="Buscar por nome, matrícula ou CPF…"
                       autocomplete="off"
            ...
            <div class="flex items-center gap-1 rounded-lg border p-1 shadow-xs border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
                <a href="{{ route('motoristas.index', array_filter(['search' => $currentSearch])) }}"
                   class="rounded-md px-3 py-1.5 text-sm font-medium transition-all duration-150
                          {{ $currentStatus === null || $currentStatus === '' ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900' : 'text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100' }}">Todos</a>
                <a href="{{ route('motoristas.index', array_filter(['search' => $currentSearch, 'status' => '1'])) }}"
                   class="rounded-md px-3 py-1.5 text-sm font-medium transition-all duration-150
                          {{ $currentStatus === '1' ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900' : 'text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100' }}">Ativos</a>
                <a href="{{ route('motoristas.index', ['search' => $currentSearch, 'status' => '0']) }}"
                   class="rounded-md px-3 py-1.5 text-sm font-medium transition-all duration-150
                          {{ $currentStatus === '0' ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900' : 'text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100' }}">Inativos</a>
            </div>
        </div>
    </form>
    ...
    <div class="mt-6 overflow-hidden rounded-xl border shadow-sm border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-900/50">
        @if($motoristas->isEmpty())
            <div class="flex flex-col items-center justify-center px-6 py-20 text-center">
                ...
                @if($currentSearch || $currentStatus !== null)
                    <h3 class="mt-4 text-base font-semibold text-zinc-900 dark:text-zinc-100">Nenhum motorista encontrado</h3>
                    <a href="{{ route('motoristas.index') }}" class="mt-5 inline-flex items-center gap-2 rounded-lg border px-4 py-2 text-sm font-medium border-zinc-200 text-zinc-700 hover:bg-zinc-50 dark:border-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-800/60">Limpar filtros</a>
                @else
                    <h3 class="mt-4 text-base font-semibold text-zinc-900 dark:text-zinc-100">Nenhum motorista cadastrado</h3>
                    <a href="{{ route('motoristas.create') }}" class="mt-5 inline-flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold bg-zinc-900 text-white hover:bg-zinc-700 dark:bg-white dark:text-zinc-900">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-slate-200 dark:border-zinc-800">
                            <th scope="col" class="px-6 py-4 text-left text-[11px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-600">Matrícula</th>
                            ...
                            <th scope="col" class="hidden px-6 py-4 text-left text-[11px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-600 sm:table-cell">CPF</th>
                            <th scope="col" class="hidden px-6 py-4 text-left text-[11px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-600 md:table-cell">Cargo</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-zinc-800/60">
                        @foreach($motoristas as $motorista)
                            <tr class="transition-colors duration-150 hover:bg-slate-50 dark:hover:bg-zinc-800/30">
                                ...
                                <td class="px-6 py-4">
                                    <p class="font-semibold text-zinc-900 dark:text-zinc-100">{{ $motorista->nome }}</p>
                                </td>
                                <td class="hidden px-6 py-4 font-mono text-sm text-zinc-600 dark:text-zinc-400 sm:table-cell">
                                    {{ $motorista->cpf }}
                                </td>
                                <td class="hidden px-6 py-4 text-zinc-600 dark:text-zinc-400 md:table-cell">
                                    {{ $motorista->cargo }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($motorista->status)
    ...
    @if(! $motoristas->isEmpty())
        <p class="mt-3 text-xs text-zinc-400 dark:text-zinc-600">
            {{ $motoristas->total() }} {{ $motoristas->total() === 1 ? 'motorista' : 'motoristas' }} no total
        </p>
    @endif
