<x-layouts.app title="Locais de Etapa">

    <div class="mt-4 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Locais de Etapa</h1>
            <p class="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">Gerencie os locais disponíveis para registro de etapas.</p>
        </div>
    ...
    {{-- Filtros --}}
    <form method="GET" action="{{ route('locais-etapa.index') }}"
    ...
    <div class="mt-4 overflow-hidden rounded-xl border shadow-sm border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-900/50">
        @if($locaisEtapa->isEmpty())
            <div class="flex flex-col items-center justify-center px-6 py-16 text-center">
                <h3 class="mt-3 text-sm font-semibold text-zinc-900 dark:text-zinc-100">Nenhum local cadastrado</h3>
                <button type="button" onclick="openCreateModal()"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-zinc-800/60">
                        @foreach($locaisEtapa as $local)
                            <tr class="hover:bg-slate-50 dark:hover:bg-zinc-800/30">
                                <td class="px-6 py-4 font-medium text-zinc-900 dark:text-zinc-100">{{ $local->nome }}</td>
                                <td class="px-6 py-4">
                                    @if($local->ativo)
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <button type="button"
                                                ...
                                                data-local="{{ json_encode(['id' => $local->getRouteKey(), 'nome' => $local->nome, 'ativo' => $local->ativo]) }}"
                                        ...
                                        <form method="POST" action="{{ route('locais-etapa.destroy', $local) }}"
                                              data-confirm="true" data-user-name="{{ $local->nome }}">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                                                    class="inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium
    ...
    {{-- Modal --}}
    <div id="local-backdrop" onclick="closeModal()"
    ...
    <div id="local-modal"
        ...
        <div class="flex items-center justify-between border-b px-6 py-4 border-slate-200 dark:border-zinc-800">
            <h3 id="modal-title" class="text-base font-semibold text-zinc-900 dark:text-zinc-100">Novo Local</h3>
        ...
        <form id="local-form" method="POST" action="{{ route('locais-etapa.store') }}" novalidate>
            @csrf
            ...
            <div class="space-y-4 px-6 py-5">
                <div class="space-y-1.5">
                    ...
                    <input id="modal-nome" type="text" name="nome" maxlength="150"
                           placeholder="Ex.: Arm Macaé - Al-13…"
    ...
    <script>
    (function () {
        var storeUrl   = '{{ route('locais-etapa.store') }}';
        var updateBase = '{{ url('locais-etapa') }}';

        function openModal() {
            document.getElementById('local-backdrop').classList.remove('hidden');
            document.getElementById('local-modal').classList.remove('hidden');
            document.body.style.overflow = 'hidden';
        }

        window.closeModal = function () {
            document.getElementById('local-backdrop').classList.add('hidden');
        ...
        window.openCreateModal = function () {
            ...
            document.getElementById('local-form').action              = storeUrl;
        ...
        window.openEditModal = function (btn) {
            var el = btn.closest('[data-local]');
            if (! el) { return; }
            var data = JSON.parse(el.dataset.local);
            ...
            document.getElementById('local-form').action              = updateBase + '/' + data.id;
