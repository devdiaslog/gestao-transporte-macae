<x-layouts.app title="Nova Ocorrência">
    ...
    <div class="mt-4 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Nova Ocorrência</h2>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Registre uma nova ocorrência de veículo.</p>
        </div>
    </div>
    ...
    <div class="mt-6 max-w-2xl rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <form method="POST" action="{{ route('ocorrencias.store') }}" class="space-y-5" novalidate>
            @csrf
                ...
                <div class="space-y-1.5">
                    ...
                    <select id="id_veiculo" name="id_veiculo"
                        ...
                        @foreach($veiculos as $veiculo)
                            <option value="{{ $veiculo->id }}" @selected(old('id_veiculo') == $veiculo->id)>
                ...
                <div class="space-y-1.5">
                    ...
                    <select id="id_tipo" name="id_tipo"
                        ...
                        @foreach($tipos as $tipo)
                            <option value="{{ $tipo->id_tipo }}" @selected(old('id_tipo') == $tipo->id_tipo)>{{ $tipo->descricao }}</option>
                        @endforeach
                    </select>
                    @error('id_tipo')
                        <p class="flex items-center gap-1.5 text-xs text-red-500 dark:text-red-400">{{ $message }}</p>
                ...
                <div class="space-y-1.5">
                    ...
                    <select id="id_responsavel" name="id_responsavel"
                        ...
                        @foreach($responsaveis as $responsavel)
                            <option value="{{ $responsavel->id_responsavel }}" @selected(old('id_responsavel') == $responsavel->id_responsavel)>
                    ...
                    <select id="id_justificativa" name="id_justificativa"
                        ...
                        @foreach($justificativas as $justificativa)
                            <option value="{{ $justificativa->id_justificativa }}"
                                    ...
                                    @selected(old('id_justificativa') == $justificativa->id_justificativa)>
            ...
            <div class="space-y-1.5">
                ...
                <textarea id="observacao" name="observacao" rows="3"
                          class="block w-full rounded-lg border px-3.5 py-2.5 text-sm shadow-xs outline-none transition-all duration-200 focus:ring-2
                                 ...
                                 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 dark:placeholder:text-zinc-500 dark:focus:border-zinc-400 dark:focus:ring-zinc-400/10">{{ old('observacao') }}</textarea>
            </div>
