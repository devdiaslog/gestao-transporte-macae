<x-layouts.app title="Nova Justificativa">
    ...
    <div class="mt-4 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Nova Justificativa</h2>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Cadastre uma nova justificativa de ocorrência.</p>
        </div>
    </div>

    <div class="mt-6 max-w-lg rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <form method="POST" action="{{ route('justificativas.store') }}" class="space-y-5" novalidate>
            @csrf
            ...
            <div class="space-y-1.5">
                ...
                <input id="descricao" type="text" name="descricao" value="{{ old('descricao') }}" autofocus
            ...
            <div class="space-y-3">
                ...
                <label class="flex cursor-pointer items-center gap-3">
                    ...
                    <input type="checkbox" id="ativo" name="ativo" value="1" @checked(old('ativo', true))
                ...
                <label class="flex cursor-pointer items-center gap-3">
                    ...
                    <input type="checkbox" id="obrigar_observacao" name="obrigar_observacao" value="1" @checked(old('obrigar_observacao', false))
                           class="h-4 w-4 rounded border-slate-300 text-zinc-900 focus:ring-zinc-900/20 dark:border-zinc-600 dark:bg-zinc-800">
                ...
                <div class="space-y-1.5">
                    ...
                    <select id="tipos" name="tipos[]" multiple size="5"
                        ...
                        @foreach($tipos as $tipo)
                            <option value="{{ $tipo->id_tipo }}" @selected(in_array($tipo->id_tipo, old('tipos', [])))>
                                {{ $tipo->descricao }}
                            </option>
                        @endforeach
                    </select>
