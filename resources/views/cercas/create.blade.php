<x-layouts.app title="Nova Cerca">
    ...
    <div class="mt-4 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Nova Cerca</h2>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Cadastre um novo local conhecido da operação.</p>
        </div>
    </div>

    <form method="POST" action="{{ route('cercas.store') }}" id="cerca-form" novalidate>
        @csrf
        <input type="hidden" name="poligono" id="poligono-input">
    ...
    (function () {
        ...
        (function () {
            try {
                var saved = JSON.parse('{{ old('poligono') }}');
                if (Array.isArray(saved)) {
                    saved.forEach(function (pt) { adicionarVertice(pt[0], pt[1]); });
                    if (saved.length > 0) { map.fitBounds(polygon ? polygon.getBounds().pad(0.2) : [saved[0]]); }
                }
            } catch (e) {}
        })();
        @endif
