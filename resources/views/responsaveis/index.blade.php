<x-layouts.app title="Responsáveis">

    <div class="mt-4 flex flex-wrap items-start justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Responsáveis</h2>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Gerencie os responsáveis internos e externos.</p>
        </div>
        <a href="{{ route('responsaveis.create') }}"
    ...
    <form id="filter-form" method="GET" action="{{ route('responsaveis.index') }}" class="mt-6">
        <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="flex min-w-56 flex-1 overflow-hidden rounded-lg border shadow-xs
                ...
                <input type="text" name="search" value="{{ $currentSearch }}" placeholder="Buscar por nome…"
                       autocomplete="off"
            ...
            <select name="tipo" onchange="this.form.submit()"
                ...
                @foreach($tipos as $tipo)
                    <option value="{{ $tipo->value }}" @selected($currentTipo === $tipo->value)>{{ $tipo->label() }}</option>
                @endforeach
            </select>
            ...
            <div class="flex items-center gap-1 rounded-lg border p-1 shadow-xs border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
                <a href="{{ route('responsaveis.index', array_filter(['search' => $currentSearch, 'tipo' => $currentTipo])) }}"
                ...
                <a href="{{ route('responsaveis.index', array_filter(['search' => $currentSearch, 'tipo' => $currentTipo, 'ativo' => '1'])) }}"
                   class="rounded-md px-3 py-1.5 text-sm font-medium transition-all duration-150
                ...
                <a href="{{ route('responsaveis.index', array_filter(['search' => $currentSearch, 'tipo' => $currentTipo, 'ativo' => '0'], fn ($v) => $v !== null)) }}"
                   class="rounded-md px-3 py-1.5 text-sm font-medium transition-all duration-150
        ...
        @if($currentSearch || $currentTipo || $currentAtivo !== null)
            <div class="mt-3 flex items-center gap-2">
                ...
                @endif
                @if($currentTipo)
                    <span class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium border-zinc-200 bg-zinc-50 text-zinc-700 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300">{{ $currentTipo }}</span>
                @endif
                <a href="{{ route('responsaveis.index') }}" class="text-xs text-zinc-400 underline underline-offset-2 hover:text-zinc-700 dark:text-zinc-600 dark:hover:text-zinc-400">Limpar</a>
            </div>
        @endif
    </form>
    ...
    <div class="mt-8 overflow-hidden rounded-xl border shadow-sm border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-900/50">
        @if($responsaveis->isEmpty())
            <div class="flex flex-col items-center justify-center px-6 py-20 text-center">
                ...
                @if($currentSearch || $currentTipo || $currentAtivo !== null)
                    <h3 class="mt-4 text-base font-semibold text-zinc-900 dark:text-zinc-100">Nenhum responsável encontrado</h3>
                    <a href="{{ route('responsaveis.index') }}" class="mt-5 inline-flex items-center gap-2 rounded-lg border px-4 py-2 text-sm font-medium border-zinc-200 text-zinc-700 hover:bg-zinc-50 dark:border-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-800/60">Limpar filtros</a>
                @else
                    <h3 class="mt-4 text-base font-semibold text-zinc-900 dark:text-zinc-100">Nenhum responsável cadastrado</h3>
                    <a href="{{ route('responsaveis.create') }}" class="mt-5 inline-flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold bg-zinc-900 text-white hover:bg-zinc-700 dark:bg-white dark:text-zinc-900">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15"/></svg>
                        Criar primeiro responsável
                    </a>
                @endif
            </div>
        @else
                            ...
                            <tr class="transition-colors duration-150 hover:bg-slate-50 dark:hover:bg-zinc-800/30">
                                ...
                                <td class="px-6 py-4">
                                    @if($responsavel->tipo->value === 'INTERNO')
                                        <span class="inline-flex items-center gap-1.5 rounded-full bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700 ring-1 ring-inset ring-blue-200 dark:bg-blue-950/40 dark:text-blue-400 dark:ring-blue-500/30">
                                            Interno
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700 ring-1 ring-inset ring-amber-200 dark:bg-amber-950/40 dark:text-amber-400 dark:ring-amber-500/30">
                                            Externo
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('responsaveis.edit', $responsavel) }}"
                                        ...
                                        <form method="POST" action="{{ route('responsaveis.destroy', $responsavel) }}"
                                              data-confirm="true" data-user-name="{{ $responsavel->nome }}">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                                                    class="inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-all duration-150
    ...
    @if(!$responsaveis->isEmpty())
        <p class="mt-3 text-xs text-zinc-400 dark:text-zinc-600">
            {{ $responsaveis->total() }} {{ $responsaveis->total() === 1 ? 'responsável' : 'responsáveis' }} no total
        </p>
    @endif
