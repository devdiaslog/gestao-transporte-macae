<x-layouts.app title="Editar Responsável">
    ...
    <div class="mt-4 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Editar Responsável</h2>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">{{ $responsavel->nome }}</p>
        </div>
    </div>

    <div class="mt-6 max-w-lg rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <form method="POST" action="{{ route('responsaveis.update', $responsavel) }}" class="space-y-5" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="space-y-1.5">
                ...
                <input id="nome" type="text" name="nome" value="{{ old('nome', $responsavel->nome) }}" autofocus
            ...
            <div class="space-y-1.5">
                ...
                <select id="tipo" name="tipo"
                    ...
                    @foreach($tipos as $tipo)
                        <option value="{{ $tipo->value }}" @selected(old('tipo', $responsavel->tipo->value) === $tipo->value)>{{ $tipo->label() }}</option>
                    @endforeach
                </select>
                @error('tipo')
                    <p class="flex items-center gap-1.5 text-xs text-red-500 dark:text-red-400">{{ $message }}</p>
            ...
            <div class="space-y-1.5">
                ...
                <div class="flex items-center gap-6">
                    <label class="flex cursor-pointer items-center gap-2.5">
                        <input type="radio" name="ativo" value="1" @checked(old('ativo', $responsavel->ativo ? '1' : '0') === '1')
                    ...
                    <label class="flex cursor-pointer items-center gap-2.5">
                        <input type="radio" name="ativo" value="0" @checked(old('ativo', $responsavel->ativo ? '1' : '0') === '0')
                               class="h-4 w-4 border-zinc-300 text-zinc-900 focus:ring-zinc-900/20 dark:border-zinc-600 dark:bg-zinc-800">
