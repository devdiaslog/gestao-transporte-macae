<x-layouts.app title="Modelos de Equipamentos">
    ...
    <div class="mt-4 flex flex-wrap items-start justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Modelos de Equipamentos</h2>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Gerencie os modelos de equipamentos do sistema.</p>
        </div>
        <a href="{{ route('modelos-equipamentos.create') }}"
    ...
    @php
        ...
        $currentTipo   = request('tipo_equipamento_id');
    @endphp

    <form id="filter-form" method="GET" action="{{ route('modelos-equipamentos.index') }}" class="mt-6">
        <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="flex min-w-56 flex-1 overflow-hidden rounded-lg border shadow-xs
                ...
                <label for="search" class="sr-only">Buscar modelos</label>
            ...
            {{-- Filtro tipo --}}
            <select name="tipo_equipamento_id"
            ...
            {{-- Exportar CSV --}}
            <a href="{{ route('modelos-equipamentos.export', array_filter(['search' => $currentSearch, 'tipo_equipamento_id' => $currentTipo, 'status' => $currentStatus])) }}"
            ...
            <div class="flex items-center gap-1 rounded-lg border p-1 shadow-xs
                        ...
                        dark:border-zinc-800 dark:bg-zinc-950">
                @php
                    $baseParams = array_filter(['search' => $currentSearch, 'tipo_equipamento_id' => $currentTipo]);
                @endphp
                <a href="{{ route('modelos-equipamentos.index', $baseParams) }}"
                ...
                <a href="{{ route('modelos-equipamentos.index', array_filter(array_merge($baseParams, ['status' => '1']))) }}"
                   class="rounded-md px-3 py-1.5 text-sm font-medium transition-all duration-150
                ...
                <a href="{{ route('modelos-equipamentos.index', array_filter(array_merge($baseParams, ['status' => '0']), fn ($v) => $v !== null)) }}"
                   class="rounded-md px-3 py-1.5 text-sm font-medium transition-all duration-150
        ...
        @if($currentSearch || $currentStatus !== null || $currentTipo)
            ...
            <div class="flex flex-col items-center justify-center px-6 py-20 text-center">
                ...
                @if($currentSearch || $currentStatus !== null || $currentTipo)
                    ...
                    <p class="mt-1.5 max-w-xs text-sm text-zinc-500 dark:text-zinc-400">
                        Nenhum modelo corresponde aos critérios informados.
                    </p>
                    <a href="{{ route('modelos-equipamentos.index') }}"
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-zinc-800/60">
                        @foreach($modelos as $modelo)
                            <tr class="transition-colors duration-150 hover:bg-slate-50 dark:hover:bg-zinc-800/30">
                                <td class="px-6 py-4">
                                    <p class="font-semibold text-zinc-900 dark:text-zinc-100">{{ $modelo->nome }}</p>
                                </td>
                                ...
                                <td class="hidden px-6 py-4 text-zinc-500 dark:text-zinc-400 sm:table-cell">
                                    <time datetime="{{ $modelo->created_at->toISOString() }}"
                                          title="{{ $modelo->created_at->format('d/m/Y H:i') }}">
                                        {{ $modelo->created_at->format('d/m/Y') }}
                                    </time>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('modelos-equipamentos.edit', $modelo) }}"
                                        ...
                                        <form method="POST" action="{{ route('modelos-equipamentos.destroy', $modelo) }}"
                                              data-confirm="true"
                                              data-user-name="{{ $modelo->nome }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                    class="inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5
    ...
    @if(!$modelos->isEmpty())
        <p class="mt-3 text-xs text-zinc-400 dark:text-zinc-600">
            {{ $modelos->total() }} {{ $modelos->total() === 1 ? 'modelo' : 'modelos' }} no total
            @if($currentSearch || $currentStatus !== null || $currentTipo)
                · filtros ativos
            @endif
        </p>
