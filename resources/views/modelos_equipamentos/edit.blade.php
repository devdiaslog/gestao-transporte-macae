<x-layouts.app title="Editar Modelo de Equipamento">
    ...
    <div class="mt-4 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Editar Modelo de Equipamento</h2>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Atualize os dados do modelo <span class="font-medium text-zinc-700 dark:text-zinc-300">{{ $modeloEquipamento->nome }}</span>.</p>
        </div>
    </div>
    ...
    <div class="mt-6 max-w-lg rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <form method="POST" action="{{ route('modelos-equipamentos.update', $modeloEquipamento) }}" class="space-y-5" novalidate>
            @csrf
            @method('PUT')

            {{-- Tipo de Equipamento --}}
            <div class="space-y-1.5">
                <label for="tipo_equipamento_id" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">
                ...
                <select
                    ...
                    class="block w-full rounded-lg border px-3.5 py-2.5 text-sm
                           ...
                           focus:ring-2
                           {{ $errors->has('tipo_equipamento_id')
                    ...
                    @foreach($tipos as $tipo)
                        <option value="{{ $tipo->id }}" @selected(old('tipo_equipamento_id', $modeloEquipamento->tipo_equipamento_id) == $tipo->id)>
                            {{ $tipo->nome }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="space-y-1.5">
                ...
                <input
                    ...
                    type="text"
                    name="nome"
                    value="{{ old('nome', $modeloEquipamento->nome) }}"
                    required
                    autofocus
                    placeholder="Ex: Cavalo 2544"
                    class="block w-full rounded-lg border px-3.5 py-2.5 text-sm
                           shadow-xs outline-none transition-all duration-200
                ...
                <div class="flex items-center gap-6">
                    <label class="flex cursor-pointer items-center gap-2.5">
                        <input type="radio" name="status" value="1"
                               @checked(old('status', $modeloEquipamento->status ? '1' : '0') === '1')
                    ...
                    <label class="flex cursor-pointer items-center gap-2.5">
                        <input type="radio" name="status" value="0"
                               @checked(old('status', $modeloEquipamento->status ? '1' : '0') === '0')
                               class="h-4 w-4 border-zinc-300 text-zinc-900 focus:ring-zinc-900/20 dark:border-zinc-600 dark:bg-zinc-800 dark:focus:ring-zinc-400/20">
