<x-layouts.app title="Subdivisões">

    {{-- ─── Page header ──────────────────────────────────────────────────────── --}}
    <div class="mt-4 flex flex-wrap items-start justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Subdivisões</h2>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Gerencie as subdivisões do sistema.</p>
        </div>
        <a href="{{ route('subdivisoes.create') }}"
    ...
    <form id="filter-form" method="GET" action="{{ route('subdivisoes.index') }}" class="mt-6">
        <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="flex min-w-56 flex-1 overflow-hidden rounded-lg border shadow-xs
                ...
                <label for="search" class="sr-only">Buscar subdivisões</label>
            ...
            <div class="flex items-center gap-1 rounded-lg border p-1 shadow-xs
                        ...
                        dark:border-zinc-800 dark:bg-zinc-950">
                <a href="{{ route('subdivisoes.index', $currentSearch ? ['search' => $currentSearch] : []) }}"
                ...
                <a href="{{ route('subdivisoes.index', array_filter(['search' => $currentSearch, 'status' => '1'])) }}"
                   class="rounded-md px-3 py-1.5 text-sm font-medium transition-all duration-150
                ...
                <a href="{{ route('subdivisoes.index', array_filter(['search' => $currentSearch, 'status' => '0'], fn ($v) => $v !== null)) }}"
                   class="rounded-md px-3 py-1.5 text-sm font-medium transition-all duration-150
            ...
            {{-- Exportar CSV --}}
            <a href="{{ route('subdivisoes.export', array_filter(['search' => $currentSearch, 'status' => $currentStatus])) }}"
            ...
            <div class="flex flex-col items-center justify-center px-6 py-20 text-center">
                ...
                @if($currentSearch || $currentStatus !== null)
                    ...
                    <p class="mt-1.5 max-w-xs text-sm text-zinc-500 dark:text-zinc-400">
                        Nenhuma subdivisão corresponde aos critérios informados.
                    </p>
                    <a href="{{ route('subdivisoes.index') }}"
                ...
                @else
                    <h3 class="mt-4 text-base font-semibold text-zinc-900 dark:text-zinc-100">
                        Nenhuma subdivisão cadastrada
                    </h3>
                    <p class="mt-1.5 max-w-xs text-sm text-zinc-500 dark:text-zinc-400">
                        Cadastre a primeira subdivisão para começar.
                    </p>
                    <a href="{{ route('subdivisoes.create') }}"
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-zinc-800/60">
                        @foreach($subDivisoes as $subDivisao)
                            <tr class="transition-colors duration-150 hover:bg-slate-50 dark:hover:bg-zinc-800/30">
                                <td class="px-6 py-4">
                                    <p class="font-semibold text-zinc-900 dark:text-zinc-100">{{ $subDivisao->nome }}</p>
                                </td>
                                <td class="px-6 py-4 text-zinc-600 dark:text-zinc-400">
                                    {{ $subDivisao->divisao?->nome ?? '—' }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($subDivisao->status)
                                ...
                                <td class="hidden px-6 py-4 text-zinc-500 dark:text-zinc-400 sm:table-cell">
                                    <time datetime="{{ $subDivisao->created_at->toISOString() }}"
                                          title="{{ $subDivisao->created_at->format('d/m/Y H:i') }}">
                                        {{ $subDivisao->created_at->format('d/m/Y') }}
                                    </time>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('subdivisoes.edit', $subDivisao) }}"
                                        ...
                                        <form method="POST" action="{{ route('subdivisoes.destroy', $subDivisao) }}"
                                              data-confirm="true"
                                              data-user-name="{{ $subDivisao->nome }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                    class="inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5
    ...
    @if(!$subDivisoes->isEmpty())
        <p class="mt-3 text-xs text-zinc-400 dark:text-zinc-600">
            {{ $subDivisoes->total() }} {{ $subDivisoes->total() === 1 ? 'subdivisão' : 'subdivisões' }} no total
            @if($currentSearch || $currentStatus !== null)
                · filtros ativos
            @endif
        </p>
