<x-layouts.app title="Editar Subdivisão">
    ...
    <div class="mt-4 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Editar Subdivisão</h2>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Atualize os dados da subdivisão <span class="font-medium text-zinc-700 dark:text-zinc-300">{{ $subDivisao->nome }}</span>.</p>
        </div>
    </div>
    ...
    <div class="mt-6 max-w-lg rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <form method="POST" action="{{ route('subdivisoes.update', $subDivisao) }}" class="space-y-5" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="space-y-1.5">
                <label for="divisao_id" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">
                    Divisão <span class="text-red-500">*</span>
                </label>
                <select
                    id="divisao_id"
                    name="divisao_id"
                    ...
                    @foreach($divisoes as $divisao)
                        <option value="{{ $divisao->id }}" @selected(old('divisao_id', $subDivisao->divisao_id) == $divisao->id)>
                            {{ $divisao->nome }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="space-y-1.5">
                ...
                <input
                    ...
                    type="text"
                    name="nome"
                    value="{{ old('nome', $subDivisao->nome) }}"
                    required
                    autofocus
                ...
                <div class="flex items-center gap-6">
                    <label class="flex cursor-pointer items-center gap-2.5">
                        <input type="radio" name="status" value="1"
                               @checked(old('status', $subDivisao->status ? '1' : '0') === '1')
                    ...
                    <label class="flex cursor-pointer items-center gap-2.5">
                        <input type="radio" name="status" value="0"
                               @checked(old('status', $subDivisao->status ? '1' : '0') === '0')
                               class="h-4 w-4 border-zinc-300 text-zinc-900 focus:ring-zinc-900/20 dark:border-zinc-600 dark:bg-zinc-800 dark:focus:ring-zinc-400/20">
