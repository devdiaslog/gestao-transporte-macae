<x-layouts.app title="Nova Subdivisão">
    ...
    <div class="mt-4 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Nova Subdivisão</h2>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Cadastre uma nova subdivisão no sistema.</p>
        </div>
    </div>
    ...
    <div class="mt-6 max-w-lg rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <form method="POST" action="{{ route('subdivisoes.store') }}" class="space-y-5" novalidate>
            @csrf
            ...
            <div class="space-y-1.5">
                ...
                <select
                    ...
                    @foreach($divisoes as $divisao)
                        <option value="{{ $divisao->id }}" @selected(old('divisao_id') == $divisao->id)>
                            {{ $divisao->nome }}
                        </option>
                    @endforeach
                </select>
