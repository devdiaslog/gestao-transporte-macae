<x-layouts.app title="Novo Tipo de Ocorrência">
    ...
    <div class="mt-4 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Novo Tipo de Ocorrência</h2>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Cadastre um novo tipo de ocorrência.</p>
        </div>
    </div>

    <div class="mt-6 max-w-lg rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <form method="POST" action="{{ route('tipos-ocorrencia.store') }}" class="space-y-5" novalidate>
            @csrf
                        ...
                        @foreach($responsaveis as $responsavel)
                            <label class="flex cursor-pointer items-center gap-3 px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-zinc-800/50">
                                <input type="checkbox"
                                       ...
                                       @checked(in_array($responsavel->id_responsavel, old('responsaveis', [])))
